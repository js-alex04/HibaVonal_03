namespace HibaVonal_03.Entities
{
    public class Maintainer : User
    {
        ...
        public ICollection<MaintainerSpecialisation> MaintenanceSpecialisation { get; set; } = new List<MaintainerSpecialisation>(); // a maintainer can have multiple specializations, but a specialization can be associated with multiple maintainers
        ...
        public Maintainer(int id, string name, string email, string password, bool isAvailable, List<MaintainerSpecialisation> maintenanceProfessions) : base(id, name, email, password, Role.Maintainer)
        {
            ...
            MaintenanceSpecialisation = maintenanceProfessions;
        }
    }
}
